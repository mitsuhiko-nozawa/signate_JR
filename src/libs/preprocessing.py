...
class Preprocessing():
    ...
    def __call__(self):
        ...
        if len(os.listdir(osp.join(self.WORK_DIR, "train"))) != self.nfolds*2 or \
            len(os.listdir(osp.join(self.WORK_DIR, "valid"))) != self.nfolds*2 or \
                len(os.listdir(osp.join(self.WORK_DIR, "test"))) != 1:
            ...
            for fold in range(self.nfolds):
                train = train_df[~(train_df[self.cv] == fold)]
                valid = train_df[train_df[self.cv] == fold]
                train_X = train.drop(columns=[self.y, self.cv])
                valid_X = valid.drop(columns=[self.y, self.cv])
                train_y = train[[self.y]]
                valid_y = valid[[self.y]]
                train_X.to_csv(osp.join(self.WORK_DIR, "train", f"train_X_{fold}.csv"), index=False)
                train_y.to_csv(osp.join(self.WORK_DIR, "train", f"train_y_{fold}.csv"), index=False)
                valid_X.to_csv(osp.join(self.WORK_DIR, "valid", f"valid_X_{fold}.csv"), index=False)
                valid_y.to_csv(osp.join(self.WORK_DIR, "valid", f"valid_y_{fold}.csv"), index=False)
