...
class date_cv_5fold_seed0(Feature):
    def create_features(self):
        ...
        for fold, (tr_ind, val_ind) in enumerate(kf.split(date_df_tr)):
            ...
            train_df.loc[train_index, self.name] = fold
        ...
        for date in date_df_tr_te["date"]:
            ...
            for fold, (tr_ind, val_ind) in enumerate(kf.split(trainNo_df)):
                ...
                if train_df.loc[train_index][self.name].nunique() != 1:
                    raise ValueError("error!")
                train_df.loc[train_index, self.name] = fold

        return train_df[[self.name]], None
